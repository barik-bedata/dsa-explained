...
class InorderWithPointer(Scene):
    def construct(self):
        ...
        # পয়েন্টার এক নোড থেকে অন্য নোডে নেওয়া + বিজলি/আলোর ঝলকানি এফেক্ট
        def move_pointer(from_node, to_node, run_time=0.6):
            animations = []
            
            # আগের নোড স্বাভাবিক অবস্থায় ফেরানো
            if from_node:
                animations.append(circles[from_node].animate.set_stroke(
                    color=NORMAL_COLOR, width=NORMAL_STROKE_WIDTH
                ))
            
            # নতুন নোডকে পয়েন্টার দিয়ে হাইলাইট করা
            if to_node:
                animations.append(circles[to_node].animate.set_stroke(
                    color=HIGHLIGHT_COLOR, width=POINTER_STROKE_WIDTH
                ))
            
            # --- বিজলি/আলোর ঝলকানি এফেক্ট (ShowPassingFlash) ---
            if from_node and to_node:
                # কোন দিক থেকে কোন দিকে আলো যাবে তা পজিশন দিয়ে নির্দিষ্ট করা হলো
                vec = positions[to_node] - positions[from_node]
                unit_vec = vec / np.linalg.norm(vec)
                start = positions[from_node] + unit_vec * NODE_RADIUS
                end = positions[to_node] - unit_vec * NODE_RADIUS
                
                # একটি উজ্জ্বল হলুদ রঙের মোটা লাইনের ঝিলিক তৈরি করা হলো
                flash_line = Line(start, end, color=YELLOW, stroke_width=8)
                animations.append(ShowPassingFlash(flash_line, time_width=0.5))
            
            if animations:
                self.play(*animations, run_time=run_time)

        # নোড প্রসেস করা + সেলেব্রেশন পপ এফেক্ট
        def process_node(node_id):
            self.play(
                # ব্যাকগ্রাউন্ড সবুজ করা
                circles[node_id].animate.set_fill(PROCESSED_COLOR, opacity=1),
                # --- সেলেব্রেশন ফ্ল্যাশ এফেক্ট ---
                Flash(
                    circles[node_id], 
                    color=GREEN,           
                    flash_radius=0.7,      
            ...
            # প্রসেস করার পর একটু বাউন্স এফেক্ট দেওয়া
            self.play(circles[node_id].animate.scale(1.15), run_time=0.1)
            self.play(circles[node_id].animate.scale(1/1.15), run_time=0.1)
        ...
        # ৮. ৫ থেকে ব্যাকে যাও ২ তে, তারপর ২ থেকে ১ তে (Backtracking 5 -> 2 -> 1)
        ...
        # ১৭. ৭ থেকে ব্যাকে যাওয়া (3 -> 7, 1 -> 3) - সবার শেষে স্বাভাবিক অবস্থা
        ...
        # শেষ নোডের পয়েন্টার স্বাভাবিক করা
        self.play(circles[1].animate.set_stroke(
            color=NORMAL_COLOR, width=NORMAL_STROKE_WIDTH
        ), run_time=0.5)
